<script>
     ...
     let subsections = [
        {
            ...
            items: [
                {label: 'Approved Safety Plan & Risk Assessment (HIRARC) on site', score: null, showInfo: false},
                {label: 'Approved SOPs', score: null, showInfo: false},
                {label: 'Daily Toolbox Briefing Records', score: null, showInfo: false},
                {label: 'Work Permit Process (Hot works, Confined Space, etc.)', score: null, showInfo: false}
            ]
        },
        {
            ...
            items: [
                ...
                {label: 'Project Supervisor/Engineer Responsible in Absence of Safety Supervisor', score: null, showInfo: false},
        ...
        },
        {
            ...
            items: [
                {label: 'Full PPE Compliance (Helmet, Harness, Vest, Boots, Gloves)', score: null, showInfo: false},
                {label: 'Workers briefed & aware of Safety SOPs', score: null, showInfo: false},
                {label: 'Workers possess valid CIDB/OSH/Safety Cards/NIOSH passport', score: null, showInfo: false},
    ...
    function calculatePercentage(subsection) {
        const validScores = subsection.items.filter(
    ...
    function calculateOverallMax(subsections) {
        return subsections.reduce(
            ...
            (max, subsection) =>
                max + subsection.items.length * MAX_SCORE_PER_ITEM,
            0
        );
    }
    ...
    <form action="" class="forms">
        ...
        {#each subsections as subsection, sIndex}
            ...
            <div class="audit-grid">
                {#each subsection.items as item, iIndex}
                    ...
                    <div class="audit-select info-wrapper">
                        <button type="button" class="info-button" on:click={() => item.showInfo = !item.showInfo}><Info/></button>
                        {#if item.showInfo}
                            <div class="info-popup">
                                <p class="info-details"><span class="one">1</span> Document exists but is lacking critical sections or less relevant to current site work.</p>
                                <p class="info-details"><span class="two">2</span> Incomplete or draft version is available; approval status unclear or still pending.</p>
                                <p class="info-details"><span class="three">3</span> Document is available but either: Outdated, Unsigned, or Partially approved.</p>
                                <p class="info-details"><span class="four">4</span> Latest and signed document is available, but accessibility is limited (e.g. only in digital format or stored but not communicated to workers).</p>
                                <p class="info-details"><span class="five">5</span> Latest version, signed and dated, clearly approved, physically available at the site office, and known to site personnel.</p>
                            </div>
                        {/if}
                    </div>
            ...
            <div class="subsection-type">
                ...
                <input type="text" class="subsection-count" value="{calculatePercentage(subsection)}%" readonly>
            </div>
        {/each}
        <hr>
        <div class="overall-summary">
            <label for="" class="overall-label heading">Total / {calculateOverallMax(subsections)}</label>
            <input type="text" class="overall-count" value="{calculateOverallTotal(subsections)}" readonly>
        </div>
        <div class="overall-summary">
            <label for="" class="overall-label heading">Overall</label>
            <input type="text" class="overall-count" value="{calculateOverallPercentage(subsections)}%" readonly>
        </div>
        <hr>
...
<style>
    * {
    ...
    .subsection-count, .overall-count {
        width: 50px;
        height: 40px;
        font-size: 14px;
        padding: 1px;
    ...
    .subsection-label, .overall-label {
        width: 100px;
    }

    .subsection-type, .overall-summary {
        display: flex;
        align-items: center;
        justify-content: center;
        gap: 15px;
